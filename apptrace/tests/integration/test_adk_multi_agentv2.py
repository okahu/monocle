...
import pytest
from common.utils import verify_inference_span
...
def setup():
    try:
        ...
        instrumentor = setup_monocle_telemetry(
            workflow_name="examples_adk_travelagent_app", 
            span_processors=span_processors,
            # monocle_exporters_list='file,blob,okahu'
        )
...
def verify_spans(memory_exporter):
    ...
    for span in spans:
        ...
        if "span.type" in span_attributes and (
             ...
             # Verify for inference spans
            verify_inference_span(
                span_attributes=span.attributes,
                span_events=span.events,
                provider_type="inference.gemini",
                model_name="gemini-2.0-flash",
                expected_endpoint="optional_endpoint"
            )
        ...
        # Verify for agentic invocation spans
        if (
        ...
        # Verify for agentic tool invocation spans
        if (
                "span.type" in span_attributes
