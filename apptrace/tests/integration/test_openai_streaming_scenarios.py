...
async def test_stream_with_retries(setup):
    ...
    # The deliberately failed attempt above is traced too, but it never
    # received a usage chunk. Scope the assertions to the successful stream.
    setup.reset()
    _chunks, text = await acollect_stream(stream, openai_chunk_text)
...
async def test_stream_model_fallback(setup):
    ...
    # The deliberately failed primary above is traced too, but it never
    # received a usage chunk. Scope the assertions to the successful stream.
    setup.reset()
    _chunks, text = await acollect_stream(stream, openai_chunk_text)
    assert text
